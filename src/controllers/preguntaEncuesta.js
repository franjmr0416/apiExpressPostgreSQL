...
const getUsersAndIdEncuesta = async(req, res) =>{
    const response = await db.query("select u.id, u.nombre, u.apellidos, age(u.nacimiento) as edad, split_part(u.email,'@',1) as noControl, u2.idencuesta from usuario u inner join usuarioencuesta u2 on u.id = u2.idusuario");
    res.json(response.rows);
};
