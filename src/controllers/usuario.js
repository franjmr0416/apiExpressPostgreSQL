...
const getTabla = async(req, res) =>{
  const response = await db.query("select u.id, u.nombre, u.apellidos, u.nacimiento, u.email, u.clave, u.direccion, u.estatus, u.idtipo, t.tipo, u.idarea, a.carreradepto from usuario u inner join tipousuario t on t.id = u.idtipo inner join area a on a.id = u.idarea;");
  res.json(response.rows);
};
//punto 4 medico
